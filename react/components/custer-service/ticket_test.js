...
export default function TicketsTest() {
  ...
  useEffect(() => {
    const fetchNewMessages = async () => {
      try {
        const response = await fetch('http://localhost:3005/api/chat_box')
        const result = await response.json()
        console.log(result)
        const newMessageCount = result.data.newMessageCount
        // const newMessageCount = 5
    ...
    const intervalId = setInterval(fetchNewMessages, 30000)
  ...
  const ChatNotification = ({ userId }) => {
    ...
    useEffect(() => {
      const fetchUnreadCount = async () => {
        try {
          const response = await axios.get(`/api/chat_box/${userId}`)
      ...
      <div className={styles.containerStyle}>
        ...
        <div className={styles.titleStyle}>
          ...
          >
            ...
            {ChatNotification > 0 && (
              <span className={styles.badge}>{ChatNotification}</span>
            )}
          </button>
        </div>
          ...
          <div>
            <AnimatePresence>
              {tickets.length === 0 ? (
                ...
                <div className={styles.RWDStyle}>
                  {currentTickets.map((ticket) => (
                    <motion.div
                      key={user_id}
                      className={styles.RWDCardStyle}
                      initial={{ opacity: 0, y: 0.9 }}
                      animate={{ opacity: 1, y: 1 }}
                      exit={{ opacity: 0, y: 0.9 }}
                      transition={{ duration: 0.5 }}
                    >
                      <div className={styles.CardRWDList}>
                        <div className={styles.CardDiv}>
                          <div className={styles.CardRWDTitle}>
                            <div className={styles.RWDTitle1}>
                              客服單編號 : {ticket.id}
                            </div>
                            <div className={styles.RWDTitle2}>
                              <div className={styles.RWDTitleP}>問題分類</div>
                              <div className={styles.RWDTitleP}>問題說明</div>
                              <div className={styles.RWDTitleP}>檢視狀態</div>
                              <div className={styles.RWDTitleP}>查看詳細</div>
                            </div>
                          </div>
                          <div className={styles.CardRWDP}>
                            <div className={styles.RWDTitleP}>
                              {ticket.category}
                            </div>
                            <div className={styles.RWDTitleP}>
                              {ticket.description}
                            </div>
                            <div className={styles.RWDTitleState}>
                              <div
                                className={
                                  ticket.status === '尚未回覆'
                                    ? styles.CsState1
