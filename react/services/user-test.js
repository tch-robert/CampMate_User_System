...
export const checkAuth = async () => {
  return await axiosInstance.get('/auth-test/check')
}
/**
...
export const login = async (loginData = {}) => {
  return await axiosInstance.post('/auth-test/login', loginData)
}
/**
 * 登出用
 */
export const logout = async () => {
  return await axiosInstance.post('/auth-test/logout', {})
}
/**
 * 載入會員id的資料用，需要登入後才能使用。此API路由會檢查JWT中的id是否符合本會員，不符合會失敗。
 */
export const getUserById = async (id = 0) => {
  return await axiosInstance.get(`/users-test/${id}`)
}
/**
 * 忘記密碼/OTP 要求一次性密碼
 */
