...
const linePayClient = createLinePayClient({
  channelId: '2006163593',
  channelSecretKey: '315e2e8698f15bce5888eadbd245f876',
...
router.get('/reserve', async (req, res) => {
  ...
  // 設定重新導向與失敗導向的網址
  const redirectUrls = {
    confirmUrl: `http://localhost:3000/rent_cart/finish`,
    cancelUrl: `http://localhost:3000/rent_cart/finish`,
  }

  // 從資料庫取得訂單資料
  const orderRecord = await P_Shop_Order.findByPk(orderId, {
    raw: true, // 只需要資料表中資料
  })
  ...
  try {
    ...
    // 在db儲存reservation資料
    const result = await P_Shop_Order.update(
      {
