...
const { C_Order } = sequelize.models
...
router.post('/create-order', authenticate, async (req, res) => {
  ...
  const dbOrder = {
    ...
    campground_id: req.body.campground_id,
  ...
  // 儲存到資料庫
  await C_Order.create(dbOrder)

  // 再把資料撈出來
  const fullOrder = await C_Order.findByPk(orderId)

  // 回傳給前端的資料
  res.json({ status: 'success', data: { order }, fulldata: { fullOrder } })
})
...
router.get('/reserve', async (req, res) => {
  ...
  // 設定重新導向與失敗導向的網址
  const redirectUrls = {
    confirmUrl: 'http://localhost:3000/campground/order',
    cancelUrl: 'http://localhost:3000/campground/order',
  }

  // 從資料庫取得訂單資料
  const orderRecord = await C_Order.findByPk(orderId, {
    raw: true, // 只需要資料表中資料
  })
  ...
  try {
    ...
    // 在db儲存reservation資料
    const result = await C_Order.update(
      {
